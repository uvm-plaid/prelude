...
import plaid.prelude.ast.{AtExpr, BVAddExpr, BVSubExpr, BVMultExpr, BVConcatExpr, Expr, MinusExpr, Num, PlusExpr, PublicExpr, TimesExpr, VectorExpr}
...
class BitVectorTermFactory extends TermFactory {
  ...
  override def toTerm(expr: Expr, idx: Option[Int] = None): Term = expr match
    ...
    case BVSubExpr(e1, e2) => termManager.mkTerm(Kind.BITVECTOR_SUB, toTerm(e1), toTerm(e2))
    case BVMultExpr(e1, e2) => termManager.mkTerm(Kind.BITVECTOR_MULT, toTerm(e1), toTerm(e2))
