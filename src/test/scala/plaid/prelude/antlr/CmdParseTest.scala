...
class CmdParseTest {
  ...
  def assignCommand(): Unit =
    assertEquals(
      AssignCmd(AtExpr(OutputExpr(Str("~")), Num(1)), Num(3)),
  ...
  def letCommand(): Unit =
    assertEquals(
      LetCmd(Identifier("x"), Num(4), List(
        AssignCmd(AtExpr(OutputExpr(Str("~")), Num(1)), Identifier("x")))),
  ...
  def letCommandScope(): Unit =
    assertEquals(
      LetCmd(Identifier("x"), Num(4), List(
          AssignCmd(AtExpr(OutputExpr(Str("~")), Num(1)), Identifier("x")),
          AssignCmd(AtExpr(OutputExpr(Str("~")), Num(2)), Identifier("x")))),
  ...
  def fullLineComments(): Unit =
    assertEquals(
      LetCmd(Identifier("x"), Num(4), List(
        AssignCmd(AtExpr(OutputExpr(Str("~")), Num(1)), Identifier("x")),
        AssignCmd(AtExpr(OutputExpr(Str("~")), Num(2)), Identifier("y")))),
      Loader.command("let x = 4 in out@1 := x;\n//out@3 := z;\nout@2 := y"))
